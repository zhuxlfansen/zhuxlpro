	...
	if(account==""){
		...
		$.post("<%=path%>/mob/getBind",{
			...
			if(data=="userIdNotExist"){
				...
				$("#content").html("<h2 style='color: green'>绑定成功！请返回微信界面上传您的作品！</h2>");
			}
		});
	}
}
...
<div class="content" data-role="content" id="content">
	<c:choose>
		<c:when test="${count>0 }">
			<h2 style='color: green'>该微信号已绑定！请返回微信界面上传您的作品！</h2>
		</c:when>
		<c:otherwise>
			<h3>请根据您分配到的账号密码完成绑定</h3>
			<div class="ui-field-contain" >
				<label style="color: red" id="msg" ></label>
			    <input type="text" id="account" class="ui-btn-inline" placeholder="输入您的账号" value="">
