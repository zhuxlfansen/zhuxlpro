...
jQuery(function($){
	if($("#form1")){
		...
		$("#name").formValidator({onshow:"* 必填",onfocus:"输入活动名称",oncorrect:"&nbsp;"}).inputValidator({min:1,max:50,onerror: "活动名称长度为1-50位"});
			...
			<div class="span9">
			   ...
			   <h3>增加活动</h3>
				<table>
					<tr>
						<th>名称:</th>
						<td><input id="name" type='text' name="name" style="height: 26px;width: 370px;"/><span id="nameTip"></span>
						</td>
					</tr>
					<tr>
						<th>内容:</th>
						<td><!-- <input id="content" type='text' name="content" style="height: 26px;"/> -->
						<textarea name="content" id="content" style="height: 100px;width: 370px;"></textarea><span id="contentTip"></span>
						</td>
					</tr>
					<tr>
						<th>开始时间:</th>
						<td><input id="startTime" type='text' style="height: 26px;" name="startTime" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm',minDate:'%y-%M-{%d+1}'})" /><span id="startTimeTip"></span>
						</td>
					</tr>
					<tr>
						<th>结束时间:</th>
						<td><input id="endTime" type="text" style="height: 26px;" name="endTime" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm',minDate:'%y-%M-{%d+1}'})"/><span id="endTimeTip"></span>
						</td>
					</tr>
					<tr>
						<th>活动地址:</th>
						...
						<select id="secCg" name="secCg" onchange="copyCategoryValue(this.value);" style="display: none;width: 160px;"></select>
						<input id="address" name="address" type='text' style="height: 26px;"/>
						<span id="addressTip"></span>
						</td>
					</tr>	
					<tr>
						<th></th>
						<td><input type="button" value="保存" onclick="add()"  class="SaveBtn" />
