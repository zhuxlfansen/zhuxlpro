	...
	$.ajax({
		...
		success : function(results) {
			...
			$.each(arr, function(i, n) {
				...
				sfArr.push(n.satisZZ);
				sfArr.push(n.satisNR);
				sfArr.push(n.satisJD);
			...
			myChart.on("click", function(param) {
				...
				$.ajax({
					...
					success : function(r) {
						...
						var li="";
						$.each(arr, function(i, n) {
							li+="<li style='margin-bottom: 10px'><span class='label label-info'>"+n.studentName+" / "+n.schoolName+" / "
								+n.majorName+"</span><label>（评分："+n.satisfaction+"分）"+(n.suggestion==''?'无评论':n.suggestion)+"</label></li>"
						});
						$("#sugUL").html("");
						$("#sugUL").html(li);
					}
...
function buttonClick(obj) {
	...
	drawPieChart(path+"/chart/"+obj.id,null);
}
...
function scoreButtonClick(obj){
	...
	drawPieChart(path+"/chart/"+chartType,null);
}
...
function showArctivityList(){
	$.post(path+'/chart/queryActivityList',{startTime:$("#startTime").val(),endTime:$("#endTime").val()},function(data) {
		...
		myChart.clear;
		myChart.dispose;
		drawPieChart(path+"/chart/"+$("#chartType").val(),null);
	});
}
